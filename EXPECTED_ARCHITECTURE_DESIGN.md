...
**定位**：统一的文档组织视图，支持基于标签的视点和基于代码关联的视点

**展示内容**：展示基于不同视点组织的 Artifact（基于 Viewpoint 特化）

**核心设计理念**：
- **响应式更新**：监听 VSCode 编辑器的文件打开事件（`onDidChangeActiveTextEditor`），自动切换视点
- **默认视点**：用户没有选择视点时，默认显示"当前代码关联文档"视点
- **视点切换**：用户可以选择不同的视点切换视图（预定义视点、自定义视点、代码关联视点）
- **统一视图**：所有视点都在同一个视点视图中展示，通过视点选择器切换
...
1. **标签（Tags）** - 标签视点的切入点
   ...
   - 标签可以是单个字符串或字符串数组
   - 标签是标签视点的切入点，用于串联和组织文档

2. **视点（Viewpoint）**
   - 视点定义了文档的组织方式
   - 视点类型：
     - **标签视点**：基于标签组合规则（必须包含、可选包含、排除）
     - **代码关联视点**：基于 ArtifactMetadata.relatedCodePaths 字段（响应式，根据当前打开的文件动态更新）
   - 每个视点定义了一个特定的文档视图角度

3. **视点视图（Viewpoint View）**
   - 支持多种视点类型（标签视点、代码关联视点）
   - 各个视点展开以树的形式展示
   - 支持多种预定义视点（生命周期、架构层次、需求管理、架构管理等）
   - 支持自定义视点（基于标签组合规则）
   - **默认视点**：当前代码关联文档视点（当用户没有选择视点时）

**职责**：
- **响应式更新**：监听编辑区文件打开事件，自动更新代码关联视点
- **视点管理**：支持视点选择、切换、配置
- **标签视点**：基于标签筛选和组织 Artifact
- **代码关联视点**：
  - **正向关联**：打开文档（Artifact）时，显示该文档关联的代码路径
  - **反向关联**：打开代码文件时，显示哪些文档关联了这个代码文件（默认视点）
- 以树形结构展示匹配视点的所有文档
- 支持自定义视点（基于标签组合规则）
- 支持视点配置管理
- 支持视点展开/折叠
...
- `ViewpointTreeDataProvider`：视点树视图数据提供者（响应式更新）
- `FileWatcher`：文件打开事件监听器（监听 `onDidChangeActiveTextEditor`）
- `ViewpointManager`：视点管理器
- `ViewpointTreeItem`：视点树项

**预定义视点**：

1. **当前代码关联文档（current-code-related）** ⭐ **默认视点**
   - 用途：显示与当前打开代码文件关联的所有文档
   - 类型：代码关联视点（响应式）
   - 触发条件：打开代码文件时自动激活
   - 展示方式：树形结构，按 viewType 分类组织
   - **默认行为**：用户没有选择视点时，自动显示此视点
...
2. **正向关联（文档 → 代码）**：
   - 当打开文档（Artifact）时：
     - 识别当前打开的文件是否为 Artifact（通过路径判断）
     - 获取该 Artifact 的元数据（ArtifactMetadata）
     - 从元数据中提取 `relatedCodePaths` 字段
     - 以树形结构展示这些代码路径（按目录层级组织）
     - 支持点击代码路径节点打开对应的代码文件

3. **反向关联（代码 → 文档）** ⭐ **默认视点**：
   - 当打开代码文件时：
     - 识别当前打开的文件是否为代码文件（通过扩展名判断）
     - 反向查找哪些 Artifact 的 `relatedCodePaths` 包含当前代码文件路径
     - 以树形结构展示关联的文档（按 viewType 分类组织）
     - 支持点击文档节点打开对应的文档
   - **自动激活**：当打开代码文件时，如果用户没有选择其他视点，自动切换到"当前代码关联文档"视点
...
**关联关系建立**（代码关联视点）：
- **文档 → 代码**：在 ArtifactMetadata 的 `relatedCodePaths` 字段中声明关联的代码路径
- **代码 → 文档**：通过查询 MetadataRepository，查找 `relatedCodePaths` 包含指定代码路径的所有 Artifact

**说明**：
- Viewpoint 是基于 Artifact 的视点特化，用于从不同角度组织 Artifact
- 视点视图统一了标签视点和代码关联视点，提供统一的文档组织视图
- 默认情况下显示"当前代码关联文档"视点，用户可以选择切换到其他视点
- 代码关联视点是响应式的，会根据编辑区打开的文件自动更新
- 各个视点展开以树的形式展示，便于浏览和管理

**显示内容示例**（默认视点 - 打开代码文件时）：
```
视点视图 [当前代码关联文档]
└── 当前文件：src/user/UserService.ts
    └── 关联文档 (3)
        ├── 📄 需求文档
        │   └── user-login.md
        ├── 🎨 设计文档
        │   └── user-module-design.md
        └── 🧪 测试文档
            └── user-service-test.md
```
...
**注意**：开发视图（Development View）已合并到视点视图（Viewpoint View）中，作为"当前代码关联文档"视点。详见 3.2.2 节"视点视图"。

#### 3.2.4 任务视图（Task View）
...
3. **Development 视图**
   - 响应式代码关联展示（根据编辑区打开的文件动态显示）
   - 代码-设计关联（基于 ArtifactMetadata.relatedCodePaths）
   - 代码审查功能
   - 规范检测（ESLint 集成）
