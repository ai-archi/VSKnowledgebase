 ...
 */
export interface TaskRelatedArtifactsService {
...
export interface AIApplicationService {
  // 暂时为空，TaskWebviewViewProvider 中未使用
}

/**
 * 任务 WebviewView Provider
 * 用于在 panel 中直接显示 Webview
 */
export class TaskWebviewViewProvider {
  ...
  resolveWebviewView(
    ...
    try {
      this.logger.info('[TaskWebviewViewProvider] Resolving webview view');
      this.logger.info(`[TaskWebviewViewProvider] Extension path: ${this.context.extensionPath}`);
      
      this.webviewView = webviewView;
      
      // 获取 webview 构建路径
      const webviewDistPath = this.getWebviewDistPath();
      this.logger.info(`[TaskWebviewViewProvider] Webview dist path: ${webviewDistPath}`);
      this.logger.info(`[TaskWebviewViewProvider] Webview dist exists: ${fs.existsSync(webviewDistPath)}`);
      
      // 检查关键文件
      const htmlPath = path.join(webviewDistPath, 'index.html');
      this.logger.info(`[TaskWebviewViewProvider] HTML path: ${htmlPath}`);
      this.logger.info(`[TaskWebviewViewProvider] HTML file exists: ${fs.existsSync(htmlPath)}`);
      ...
      // 加载 HTML
      this.logger.info('[TaskWebviewViewProvider] Loading webview HTML content...');
      const html = this.getWebviewContent(webviewView.webview as any);
      (webviewView.webview as any).html = html;
      this.logger.info('[TaskWebviewViewProvider] Webview HTML loaded successfully');
      ...
      webviewView.webview.onDidReceiveMessage(
        async (message) => {
          try {
            ...
            this.logger.error('[TaskWebviewViewProvider] Error in message handler', error);
          }
        },
        null,
      ...
      webviewView.onDidChangeVisibility(() => {
        this.logger.info(`[TaskWebviewViewProvider] Webview visibility changed: ${webviewView.visible}`);
      ...
      this.logger.info('[TaskWebviewViewProvider] Webview view resolved successfully');
    } catch (error: any) {
      const errorMessage = error?.message || String(error);
      const errorStack = error?.stack || '';
      this.logger.error('[TaskWebviewViewProvider] Failed to resolve webview view', {
      ...
      try {
        ...
        this.logger.error('[TaskWebviewViewProvider] Failed to set error HTML', htmlError);
      }
    }
  }
    ...
    this.fileWatcherDisposable = vscode.window.onDidChangeActiveTextEditor(
      async (editor) => {
        if (editor && this.webviewView?.visible) {
          this.logger.info('[TaskWebviewViewProvider] Active editor changed, notifying webview');
          this.notifyFileChanged();
        }
      }
    );
  ...
  private async notifyFileChanged(): Promise<void> {
    ...
    try {
      ...
      this.logger.error('[TaskWebviewViewProvider] Failed to notify file changed', error);
    }
  }
  ...
  private async handleWebviewMessage(
    ...
    this.logger.info(`[TaskWebviewViewProvider] Received message: ${message.method}`, {
      id: message.id,
    ...
    try {
      ...
      if (message.method === 'webviewError') {
        this.logger.error('[TaskWebviewViewProvider] Webview reported error:', message.params);
        return;
      }
      ...
      this.logger.error(
        `[TaskWebviewViewProvider] Error handling message: ${message.method}`,
        {
  ...
  private async getRelatedFiles(): Promise<any[]> {
    ...
    if (isTaskFile) {
      ...
      this.logger.info('[TaskWebviewViewProvider] Detected task solution file, using task file', {
    ...
    // 处理代码文件：获取关联的文档
    // 获取关联的 Artifacts
    // 优先使用 taskRelatedArtifactsService，如果不存在则直接使用 artifactService
    let artifactsResult: { success: boolean; value?: any[]; error?: any };
    
    if (this.taskRelatedArtifactsService && typeof this.taskRelatedArtifactsService.getRelatedArtifacts === 'function') {
      // 使用 TaskRelatedArtifactsService
      artifactsResult = await this.taskRelatedArtifactsService.getRelatedArtifacts(relativePath);
    } else {
      // 直接使用 ArtifactApplicationService
      this.logger.info('[TaskWebviewViewProvider] Using artifactService.findArtifactsByCodePath directly');
      try {
        ...
        this.logger.error('[TaskWebviewViewProvider] Error finding artifacts by code path', error);
        artifactsResult = {
          success: false,
          error: error,
        };
    ...
    if (!artifactsResult.success || !artifactsResult.value) {
      this.logger.warn('[TaskWebviewViewProvider] No related artifacts found', {
        relativePath,
  ...
  private async getTaskRelatedFilesByPath(vaultId: string, relativeTaskPath: string): Promise<any[]> {
    try {
      ...
      this.logger.error('[TaskWebviewViewProvider] Failed to get task related files by path', error);
      return [];
    }
  }
    ...
    if (artifactsResult.success && artifactsResult.value && artifactsResult.value.length > 0) {
      for (const artifactId of artifactsResult.value) {
        ...
        if (artifactId.startsWith('file:')) {
          ...
          // 没有默认 vaultId，跳过
          this.logger.warn('[TaskWebviewViewProvider] Artifact ID without vault prefix and no default vaultId, skipping', { artifactId });
        }
      }
    }
  ...
  private async getTaskRelatedFiles(taskFilePath: string): Promise<any[]> {
    ...
    if (!extracted) {
      this.logger.warn('[TaskWebviewViewProvider] Invalid task file path format', { taskFilePath });
      return [];
    }

    this.logger.info('[TaskWebviewViewProvider] Extracted vault and path from task file', {
      taskFilePath,
  ...
  private async getTaskRelatedFilesByTaskId(taskId: string): Promise<any[]> {
    this.logger.info('[TaskWebviewViewProvider] getTaskRelatedFilesByTaskId called', { taskId });
    try {
      // 获取任务列表，找到对应的任务
      const tasksResult = await this.taskService.listTasks();
      if (!tasksResult.success) {
        this.logger.warn('[TaskWebviewViewProvider] Failed to list tasks', tasksResult.error);
        return [];
      }
      if (!tasksResult.value) {
        this.logger.warn('[TaskWebviewViewProvider] Tasks list is empty');
        return [];
      }

      this.logger.info('[TaskWebviewViewProvider] Found tasks', { count: tasksResult.value.length });
      const task = tasksResult.value.find(t => t.id === taskId);
      if (!task) {
        this.logger.warn('[TaskWebviewViewProvider] Task not found', { taskId, availableTaskIds: tasksResult.value.map(t => t.id) });
        return [];
      }

      this.logger.info('[TaskWebviewViewProvider] Task found', { taskId, artifactPath: task.artifactPath, vaultId: task.vaultId });

      // 获取 vault 信息
      const vaultResult = await this.vaultService.getVault(task.vaultId);
      if (!vaultResult.success || !vaultResult.value) {
        this.logger.warn('[TaskWebviewViewProvider] Vault not found', { vaultId: task.vaultId });
        return [];
      }

      // 获取任务的关联文档和代码路径
      this.logger.info('[TaskWebviewViewProvider] Getting related artifacts and code paths', {
      ...
      this.logger.info('[TaskWebviewViewProvider] Related artifacts result', {
      ...
      this.logger.info('[TaskWebviewViewProvider] Related code paths result', {
      ...
      this.logger.info('[TaskWebviewViewProvider] Returning related files', {
      ...
      this.logger.error('[TaskWebviewViewProvider] Failed to get task related files by taskId', error);
      return [];
    }
  }
    ...
    try {
      ...
      for (let i = 0; i < updateResults.length; i++) {
        ...
        if (!updateResult.success) {
          this.logger.warn('[TaskWebviewViewProvider] Failed to update task relations', {
            taskId,
            updateIndex: i,
      ...
      this.logger.error('[TaskWebviewViewProvider] Failed to update task related files', error);
      return {
        success: false,
  ...
  private async createTask(params: any): Promise<any> {
    ...
    if (params.relatedFiles && params.relatedFiles.length > 0) {
      ...
      this.logger.info('[TaskWebviewViewProvider] Saving task relations', {
        taskId: task.id,
      ...
      for (let i = 0; i < updateResults.length; i++) {
        const updateResult = updateResults[i];
        if (!updateResult.success) {
          this.logger.warn('[TaskWebviewViewProvider] Failed to save task relations', {
            taskId: task.id,
            updateIndex: i,
            error: updateResult.error,
          });
    ...
    try {
      ...
      this.logger.warn('[TaskWebviewViewProvider] Failed to refresh task tree view', error);
    }
  ...
  private async getTaskTemplates(vaultId?: string): Promise<any[]> {
    this.logger.info(`[TaskWebviewViewProvider] getTaskTemplates called with vaultId: ${vaultId || 'undefined (all vaults)'}`);
    
    const result = await this.taskService.getTaskTemplates(vaultId);
    
    if (!result.success) {
      this.logger.error('[TaskWebviewViewProvider] Failed to get task templates', result.error);
      return [];
    }
    if (!result.value) {
      return [];
    ...
    this.logger.info(`[TaskWebviewViewProvider] getTaskTemplates returned ${templates.length} templates`);
    ...
    this.logger.info(`[TaskWebviewViewProvider] Mapped templates:`, mappedTemplates.map((t: any) => `${t.id} (${t.name})`).join(', '));
  ...
  private async listVaults(): Promise<any[]> {
    ...
    if (!result.success) {
      this.logger.warn('[TaskWebviewViewProvider] Failed to list vaults', result.error);
      return [];
    }
      ...
      async (message: any) => {
        if (message.method === 'close') {
          ...
          try {
            ...
            this.logger.error('[TaskWebviewViewProvider] Failed to list workspace files', error);
            panel.webview.postMessage({
              id: message.id,
              method: message.method,
              error: {
          ...
          try {
            ...
            this.logger.error('[TaskWebviewViewProvider] Error getting task templates in dialog', error);
            panel.webview.postMessage({
              id: message.id,
              method: message.method,
              error: {
          ...
          // 未处理的消息
          this.logger.warn(`[TaskWebviewViewProvider] Unhandled message in create task dialog: ${message.method}`);
          panel.webview.postMessage({
            id: message.id,
            method: message.method,
            error: {
      ...
      async (message: any) => {
        if (message.method === 'close') {
          ...
          try {
            ...
            this.logger.error('[TaskWebviewViewProvider] Error getting related artifacts', error);
            panel.webview.postMessage({
              id: message.id,
              method: message.method,
              error: {
          ...
          try {
            ...
            this.logger.error('[TaskWebviewViewProvider] Error getting related code paths', error);
            panel.webview.postMessage({
              id: message.id,
              method: message.method,
              error: {
          ...
          try {
            ...
            this.logger.error('[TaskWebviewViewProvider] Error updating related artifacts', error);
            panel.webview.postMessage({
              id: message.id,
              method: message.method,
              error: {
          ...
          try {
            ...
            this.logger.error('[TaskWebviewViewProvider] Error updating related code paths', error);
            panel.webview.postMessage({
              id: message.id,
              method: message.method,
              error: {
          ...
          try {
            ...
            this.logger.error('[TaskWebviewViewProvider] Error handling message in task edit relations dialog', error);
            panel.webview.postMessage({
              id: message.id,
              method: message.method,
              error: {
  ...
  private getWebviewContent(webview: vscode.Webview): string {
    try {
      ...
      this.logger.info(`[TaskWebviewViewProvider] Reading HTML from: ${htmlPath}`);

      if (!fs.existsSync(htmlPath)) {
        this.logger.warn(`[TaskWebviewViewProvider] HTML file not found: ${htmlPath}`);
        return `
          <!DOCTYPE html>
          <html>
            <head>
      ...
      let html = fs.readFileSync(htmlPath, 'utf-8');
      this.logger.info(`[TaskWebviewViewProvider] HTML file read successfully, length: ${html.length}`);
      ...
      const initialDataScript = `
        <script>
          ...
          window.initialData.view = 'task-panel';
        </script>
      `;
      ...
      html = html.replace(
        ...
        (match: string, attr: string, resourcePath: string) => {
          ...
          if (fs.existsSync(resourceFile)) {
            ...
            this.logger.warn(`[TaskWebviewViewProvider] Resource not found: ${resourcePath} (resolved to: ${resourceFile})`);
      ...
      if (missingResourceCount > 0) {
        this.logger.warn(`[TaskWebviewViewProvider] ${missingResourceCount} resources not found:`, missingResources.slice(0, 10));
      }
      this.logger.info(`[TaskWebviewViewProvider] Resource replacement: ${resourceReplaceCount} replaced, ${missingResourceCount} missing`);
      ...
      this.logger.error('[TaskWebviewViewProvider] Error in getWebviewContent', error);
      throw error;
    }
  }
  ...
  private getWebviewDistPath(): string {
    try {
      ...
      this.logger.info(`[TaskWebviewViewProvider] Webview paths - Extension: ${webviewPathInExtension} (exists: ${existsInExtension}), Source: ${webviewPathInSource} (exists: ${existsInSource})`);
      ...
      if (existsInExtension) {
        ...
        this.logger.warn(`[TaskWebviewViewProvider] Neither webview path exists, using extension path as fallback`);
        return webviewPathInExtension;
      }
    } catch (error: any) {
      this.logger.error('[TaskWebviewViewProvider] Error getting webview dist path', error);
      // 返回一个默认路径，即使可能不存在
  ...
  private async generateStepPrompt(
    ...
    try {
      ...
      // 获取任务的关联文件
      this.logger.info('[TaskWebviewViewProvider] Getting related files for prompt generation', {
        taskId,
        vaultId: task.vaultId,
        artifactPath: task.artifactPath,
      });
      ...
      this.logger.info('[TaskWebviewViewProvider] Related files retrieved', {
        count: relatedFiles.length,
        files: relatedFiles.map(f => ({ id: f.id, name: f.name, type: f.type })),
      });
      ...
      this.logger.info('[TaskWebviewViewProvider] Related files categorized', {
  ...
  private async notifyTaskChanged(): Promise<void> {
    ...
    try {
      ...
      this.logger.error('[TaskWebviewViewProvider] Failed to notify task changed', error);
    }
  }
