# SQLite + FTS5 + MCP Server 知识库搜索方案

## 📋 一、方案概述

### 1.1 目标

在 MCP Server 中实现基于 SQLite FTS5 的轻量级本地知识库搜索功能，提供：
- ✅ 快速全文搜索（<5ms 响应时间）
- ✅ 支持中文搜索（unicode61 tokenizer）
- ✅ 返回本地文件路径（便于 AI 工具引用）
- ✅ 高亮片段返回（snippet 功能）
- ✅ 无需外部服务（纯本地实现）
- ✅ 与现有 IPC 桥接架构兼容

### 1.2 设计原则

1. **独立性**：MCP Server 拥有独立的 SQLite 数据库，不依赖 VS Code 扩展的数据库
2. **轻量级**：使用 SQLite FTS5，无需向量库或外部服务
3. **可扩展**：未来可升级为混合搜索（FTS5 + 向量检索）
4. **兼容性**：保留现有 IPC 桥接功能，新增本地搜索作为补充

---

## 🏗️ 二、架构设计

### 2.1 整体架构

```
┌─────────────────────────────────────┐
│   外部 MCP Client (Cursor)           │
└──────────────┬──────────────────────┘
               │ stdio (JSON-RPC)
               │
┌──────────────▼──────────────────────┐
│   MCP Server (独立进程)              │
│   ┌──────────────────────────────┐  │
│   │  MCP 协议处理层               │  │
│   └──────────┬───────────────────┘  │
│              │                       │
│   ┌──────────▼───────────────────┐  │
│   │  工具路由层                    │  │
│   │  - IPC 桥接工具 (现有)        │  │
│   │  - 本地 FTS5 搜索工具 (新增)  │  │
│   └──────────┬───────────────────┘  │
│              │                       │
│   ┌──────────▼───────────────────┐  │
│   │  KnowledgeBaseDB (新增)       │  │
│   │  - SQLite + FTS5              │  │
│   │  - 文档索引管理               │  │
│   │  - 全文搜索                   │  │
│   └──────────┬───────────────────┘  │
│              │                       │
│   ┌──────────▼───────────────────┐  │
│   │  DocumentIndexer (新增)       │  │
│   │  - 扫描 .architool 目录       │  │
│   │  - 解析文档内容               │  │
│   │  - 同步索引                   │  │
│   └──────────────────────────────┘  │
└──────────────┬──────────────────────┘
               │ 文件系统访问
               │
┌──────────────▼──────────────────────┐
│   .architool/ 目录                   │
│   ├── {vault-name}/                 │
│   │   ├── *.md                      │
│   │   ├── *.archimate               │
│   │   └── .metadata/                │
│   └── cache/                        │
│       └── knowledge.db (SQLite)     │
└─────────────────────────────────────┘
```

### 2.2 数据流

**搜索流程**：
1. MCP Client 发送 `tools/call` 请求（`search_knowledge_base_fts5`）
2. MCP Server 路由到本地 FTS5 搜索工具
3. KnowledgeBaseDB 执行 FTS5 查询
4. 返回搜索结果（包含路径、片段、元数据）

**索引同步流程**：
1. MCP Server 启动时检查索引是否需要更新
2. DocumentIndexer 扫描 `.architool/` 目录
3. 解析文档内容和元数据
4. 批量更新 SQLite 数据库

---

## 💾 三、数据库设计

### 3.1 数据库位置

```
~/.architool/cache/knowledge.db
```

**选择理由**：
- 与 VS Code 扩展的 SQLite 数据库分离（避免冲突）
- 使用系统级缓存目录（跨工作区共享）
- 便于清理和重建

### 3.2 表结构

#### 3.2.1 documents 表（主表）

```sql
CREATE TABLE documents (
  id TEXT PRIMARY KEY,                    -- 文档唯一 ID（基于文件路径的 hash）
  path TEXT NOT NULL,                     -- 文档完整路径（相对于工作区根目录）
  vault_name TEXT,                        -- Vault 名称
  content TEXT NOT NULL,                  -- 文档全文内容
  metadata TEXT,                          -- JSON 格式的元数据
  file_type TEXT,                         -- 文件类型（md, archimate, etc.）
  created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
  updated_at DATETIME DEFAULT CURRENT_TIMESTAMP
);

CREATE INDEX documents_path_index ON documents (path);
CREATE INDEX documents_vault_index ON documents (vault_name);
```

#### 3.2.2 documents_fts 表（FTS5 虚拟表）

```sql
CREATE VIRTUAL TABLE documents_fts USING fts5(
  content,                                -- 可搜索的全文内容
  path UNINDEXED,                        -- 不索引路径（仅存储）
  metadata UNINDEXED,                    -- 不索引元数据（仅存储）
  content='documents',                    -- 关联主表
  content_rowid='id',                    -- 使用 id 作为 rowid
  tokenize='unicode61'                   -- 支持中文分词
);
```

#### 3.2.3 触发器（自动同步）

```sql
-- 插入触发器
CREATE TRIGGER documents_fts_insert AFTER INSERT ON documents BEGIN
  INSERT INTO documents_fts(rowid, content)
  VALUES (new.id, new.content);
END;

-- 更新触发器
CREATE TRIGGER documents_fts_update AFTER UPDATE ON documents BEGIN
  DELETE FROM documents_fts WHERE rowid = old.id;
  INSERT INTO documents_fts(rowid, content)
  VALUES (new.id, new.content);
END;

-- 删除触发器
CREATE TRIGGER documents_fts_delete AFTER DELETE ON documents BEGIN
  DELETE FROM documents_fts WHERE rowid = old.id;
END;
```

### 3.3 元数据格式

```json
{
  "title": "文档标题",
  "vaultName": "vault-name",
  "type": "document|design|development|test",
  "category": "分类",
  "tags": ["tag1", "tag2"],
  "author": "作者",
  "createdAt": "2024-01-01T00:00:00Z",
  "updatedAt": "2024-01-01T00:00:00Z"
}
```

---

## 🔍 四、搜索功能设计

### 4.1 FTS5 搜索语法

**基本搜索**：
```sql
SELECT path, snippet(documents_fts, '<b>', '</b>', '...', -1, 64) AS snippet
FROM documents_fts
WHERE documents_fts MATCH '微服务'
ORDER BY rank
LIMIT 20;
```

**高级搜索**：
- `"精确短语"` - 短语匹配
- `keyword1 OR keyword2` - OR 查询
- `keyword1 AND keyword2` - AND 查询
- `keyword1 NOT keyword2` - 排除查询
- `prefix*` - 前缀匹配

### 4.2 搜索 API 设计

```typescript
interface SearchOptions {
  query: string;                    // 搜索查询（FTS5 语法）
  vaultName?: string;               // 可选：过滤特定 vault
  limit?: number;                   // 结果数量限制（默认 20）
  snippetLength?: number;           // 片段长度（默认 64）
  highlightPrefix?: string;         // 高亮前缀（默认 '<b>'）
  highlightSuffix?: string;         // 高亮后缀（默认 '</b>'）
}

interface SearchResult {
  path: string;                     // 文档路径（相对路径）
  title?: string;                   // 文档标题
  snippet: string;                  // 高亮片段
  score?: number;                   // 相关性评分（FTS5 rank）
  metadata?: any;                   // 元数据
}
```

### 4.3 MCP 工具定义

```typescript
{
  name: 'search_knowledge_base_fts5',
  description: '使用 SQLite FTS5 全文搜索本地知识库。支持中文搜索，返回文档路径和高亮片段。',
  inputSchema: {
    type: 'object',
    properties: {
      query: {
        type: 'string',
        description: '搜索查询（支持 FTS5 语法：短语、AND、OR、NOT、前缀匹配）'
      },
      vaultName: {
        type: 'string',
        description: '可选：过滤特定 vault'
      },
      limit: {
        type: 'number',
        description: '结果数量限制（默认 20）'
      }
    },
    required: ['query']
  }
}
```

---

## 📦 五、文档索引设计

### 5.1 索引策略

**全量索引**：
- MCP Server 启动时执行（如果数据库不存在或需要重建）
- 扫描所有 `.architool/{vault-name}/` 目录
- 排除 `.metadata/` 和系统目录

**增量索引**：
- 定期检查文件修改时间（可选）
- 仅更新变更的文件

**索引文件类型**：
- `.md` - Markdown 文档
- `.archimate` - ArchiMate 架构文件（提取文本内容）
- `.yml` / `.yaml` - YAML 元数据文件（可选）

### 5.2 文档解析

**Markdown 文件**：
```typescript
// 提取内容
- 移除 frontmatter（如果存在）
- 提取正文内容
- 保留标题结构（用于上下文）
```

**ArchiMate 文件**：
```typescript
// 提取内容
- 解析 XML 结构
- 提取元素名称和描述
- 提取关系信息
```

**元数据提取**：
```typescript
// 从 .metadata/{id}.metadata.yml 读取
- 标题、分类、标签
- 关联关系
- 作者、创建时间等
```

### 5.3 索引器实现

```typescript
class DocumentIndexer {
  // 扫描 .architool 目录
  async scanArchitoolDirectory(rootPath: string): Promise<Document[]> {}
  
  // 解析单个文档
  async parseDocument(filePath: string): Promise<KnowledgeDocument> {}
  
  // 批量索引
  async indexDocuments(docs: KnowledgeDocument[]): Promise<void> {}
  
  // 检查是否需要重建索引
  async needsRebuild(): Promise<boolean> {}
}
```

---

## 🔌 六、MCP Server 集成

### 6.1 工具路由策略

**双模式支持**：

1. **IPC 桥接模式**（现有）：
   - `search_knowledge_base` → 通过 IPC 调用 VS Code 扩展
   - `get_documents_for_code` → 通过 IPC 调用 VS Code 扩展

2. **本地 FTS5 模式**（新增）：
   - `search_knowledge_base_fts5` → 直接使用 SQLite FTS5
   - 不依赖 VS Code 扩展运行

**优势**：
- 即使 VS Code 扩展未运行，MCP Server 仍可提供搜索功能
- 响应速度更快（无需 IPC 通信）
- 降低对扩展的依赖

### 6.2 代码结构

```
packages/architool-mcp-server/
├── src/
│   ├── mcp-server.ts              # 主入口（现有）
│   ├── knowledge-base-db.ts       # SQLite FTS5 数据库模块（新增）
│   ├── document-indexer.ts       # 文档索引器（新增）
│   └── tools/
│       ├── ipc-tools.ts          # IPC 桥接工具（现有）
│       └── fts5-search-tool.ts   # FTS5 搜索工具（新增）
├── package.json
└── tsconfig.json
```

### 6.3 初始化流程

```typescript
async function initializeMCPServer() {
  // 1. 初始化知识库数据库
  const db = new KnowledgeBaseDB('~/.architool/cache/knowledge.db');
  db.initialize();
  
  // 2. 检查并重建索引（如果需要）
  const indexer = new DocumentIndexer(db);
  if (await indexer.needsRebuild()) {
    await indexer.rebuildIndex();
  }
  
  // 3. 注册 MCP 工具
  server.setRequestHandler(ListToolsRequestSchema, async () => {
    return {
      tools: [
        // IPC 桥接工具（现有）
        { name: 'search_knowledge_base', ... },
        { name: 'get_documents_for_code', ... },
        // FTS5 本地搜索工具（新增）
        { name: 'search_knowledge_base_fts5', ... }
      ]
    };
  });
}
```

---

## 🚀 七、实现步骤

### 阶段 1：核心数据库模块（优先级：高）

1. ✅ 创建 `KnowledgeBaseDB` 类
   - SQLite 数据库初始化
   - FTS5 虚拟表创建
   - 触发器设置
   - 基本 CRUD 操作

2. ✅ 实现搜索功能
   - FTS5 查询
   - snippet 高亮
   - 结果格式化

### 阶段 2：文档索引器（优先级：高）

3. ✅ 创建 `DocumentIndexer` 类
   - 扫描 `.architool/` 目录
   - 解析 Markdown 文件
   - 解析 ArchiMate 文件
   - 读取元数据文件

4. ✅ 实现索引同步
   - 全量索引
   - 增量更新
   - 索引重建检测

### 阶段 3：MCP 工具集成（优先级：中）

5. ✅ 创建 FTS5 搜索工具
   - 实现工具处理函数
   - 参数验证
   - 错误处理

6. ✅ 集成到 MCP Server
   - 注册新工具
   - 初始化流程
   - 生命周期管理

### 阶段 4：优化和测试（优先级：低）

7. ⏳ 性能优化
   - 索引缓存
   - 查询优化
   - 批量操作

8. ⏳ 测试覆盖
   - 单元测试
   - 集成测试
   - 性能测试

---

## 📊 八、性能指标

### 8.1 预期性能

| 指标 | 目标值 | 说明 |
|------|--------|------|
| 搜索响应时间 | < 5ms | 单次 FTS5 查询 |
| 索引速度 | > 100 docs/s | 批量索引 |
| 数据库大小 | < 100MB | 10,000 文档 |
| 内存占用 | < 50MB | MCP Server 进程 |

### 8.2 优化策略

1. **WAL 模式**：启用 SQLite WAL 模式提高并发性能
2. **批量插入**：使用事务批量插入文档
3. **索引缓存**：缓存常用查询结果（可选）
4. **增量更新**：仅更新变更的文件

---

## 🔮 九、未来扩展

### 9.1 混合搜索（FTS5 + 向量检索）

**架构**：
```
搜索请求
  ├─→ FTS5 搜索（关键词匹配）
  └─→ 向量搜索（语义相似度）
       └─→ 结果融合（加权排序）
```

**实现**：
- 使用 BGE-M3 或类似模型生成向量
- 使用 FAISS 或 SQLite 向量扩展存储向量
- 融合 FTS5 和向量搜索结果

### 9.2 高级功能

1. **语义搜索**：基于向量相似度的语义搜索
2. **多语言支持**：支持英文、中文等多种语言
3. **实时索引**：监听文件系统变化，实时更新索引
4. **搜索历史**：记录搜索历史，提供搜索建议

---

## 📝 十、技术选型

### 10.1 依赖包

```json
{
  "dependencies": {
    "@modelcontextprotocol/sdk": "^1.24.3",  // MCP SDK（已有）
    "better-sqlite3": "^9.0.0"                // SQLite 驱动（新增）
  },
  "devDependencies": {
    "@types/better-sqlite3": "^7.6.0",       // TypeScript 类型（新增）
    "@types/node": "^20.0.0",                 // Node.js 类型（已有）
    "typescript": "^5.0.0"                    // TypeScript（已有）
  }
}
```

### 10.2 为什么选择 better-sqlite3？

- ✅ **同步 API**：简化代码，无需 async/await
- ✅ **高性能**：比 sqlite3 快 2-3 倍
- ✅ **TypeScript 支持**：完整的类型定义
- ✅ **活跃维护**：社区活跃，更新及时

---

## ⚠️ 十一、注意事项

### 11.1 数据一致性

**问题**：MCP Server 的索引可能与 VS Code 扩展的数据不同步

**解决方案**：
1. MCP Server 启动时检查索引时间戳
2. 如果索引过期，自动重建
3. 提供手动重建索引的命令（可选）

### 11.2 多工作区支持

**问题**：MCP Server 是全局的，但知识库是工作区级别的

**解决方案**：
1. 索引所有工作区的 `.architool/` 目录
2. 在搜索结果中标识 vault 名称
3. 支持按 vault 过滤

### 11.3 文件路径处理

**问题**：不同操作系统的路径格式不同

**解决方案**：
1. 统一使用相对路径（相对于工作区根目录）
2. 使用 `path.posix` 标准化路径
3. 在搜索结果中返回标准化路径

---

## ✅ 十二、验收标准

### 12.1 功能验收

- [ ] 能够索引 `.architool/` 目录下的所有文档
- [ ] 支持中文全文搜索
- [ ] 返回文档路径和高亮片段
- [ ] 搜索响应时间 < 5ms
- [ ] 支持 FTS5 高级语法（AND、OR、NOT、短语）

### 12.2 集成验收

- [ ] MCP Client 能够调用 `search_knowledge_base_fts5` 工具
- [ ] 搜索结果格式符合 MCP 协议规范
- [ ] 错误处理完善，不会导致 MCP Server 崩溃

### 12.3 性能验收

- [ ] 10,000 文档索引时间 < 2 分钟
- [ ] 单次搜索响应时间 < 5ms
- [ ] 数据库文件大小合理（< 100MB for 10K docs）

---

## 📚 十三、参考资料

- [SQLite FTS5 文档](https://www.sqlite.org/fts5.html)
- [better-sqlite3 文档](https://github.com/WiseLibs/better-sqlite3)
- [MCP 协议规范](https://modelcontextprotocol.io/)
- [FTS5 查询语法](https://www.sqlite.org/fts5.html#fts5_query_syntax)

---

## 🎯 总结

本方案提供了一个**轻量级、高性能、易扩展**的本地知识库搜索解决方案：

1. **独立性**：MCP Server 拥有独立的 SQLite 数据库，不依赖 VS Code 扩展
2. **性能**：使用 FTS5 实现毫秒级搜索响应
3. **兼容性**：保留现有 IPC 桥接功能，新增本地搜索作为补充
4. **可扩展性**：未来可升级为混合搜索（FTS5 + 向量检索）

**下一步**：按照实现步骤，从阶段 1 开始逐步实现。

