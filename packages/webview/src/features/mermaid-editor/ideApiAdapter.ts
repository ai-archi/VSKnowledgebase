/**
 * IDE API Adapter
 * 适配 mermaid-editor 的通信方式到 ExtensionService（支持多 IDE）
 */
...
export async function fetchDiagram() {
  try {
    ...
    console.error('[ideApiAdapter] fetchDiagram error:', error);
    throw error;
  }
}
...
export async function loadMermaid(): Promise<void> {
  try {
    ...
    console.error('[ideApiAdapter] loadMermaid error:', error);
    throw error;
  }
}
...
export async function renderMermaid(source: string): Promise<void> {
  try {
    ...
    console.error('[ideApiAdapter] renderMermaid error:', error);
    throw error;
  }
}
...
export async function saveDiagram(diagram: any) {
  try {
    ...
    console.error('[ideApiAdapter] saveDiagram error:', error);
    throw error;
  }
}
